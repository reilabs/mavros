    ...
    fn try_convert_mavros_intrinsic(
        ...
        match name {
            ...
            "spread_inner" => {
                ...
                let value = self.convert_expression(&call.arguments[0], b).unwrap();
                let result = b.block(self.current_block).spread_with_bits(value, Some(bits as u8));
                Some(result)
            }
            "unspread_inner" => {
                let bits = Self::extract_const_u32(&call.arguments[1]);
                ...
                let value = self.convert_expression(&call.arguments[0], b).unwrap();
                let (odd, even) = b.block(self.current_block).unspread_with_bits(value, Some(bits as u8));
