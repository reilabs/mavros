...
fn get_or_create_drop_fn(
    ...
    let fn_id = match &ty.expr {
        TypeExpr::WitnessOf(inner) => match &inner.expr {
            TypeExpr::Field => ad_fns.get_drop_fn(llssa),
            _ => panic!("WitnessOf({}) is not supported yet", inner),
        },
...
fn generate_all_drop_functions(llssa: &mut LLSSA, drop_fns: &[DropFnEntry]) {
    for entry in drop_fns {
        let func = match &entry.hlssa_type.expr {
            ...
            // WitnessOf(Field) points to ad_drop, whose body is generated by generate_all_ad_functions
            TypeExpr::WitnessOf(inner) => match &inner.expr {
                TypeExpr::Field => continue,
                _ => panic!("WitnessOf({}) is not supported yet", inner),
            },
