            ...
            for instruction in block.take_instructions() {
                let new = match instruction {
                    ...
                    OpCode::Cast {
                        ...
                        let r_wt = function_wt.get_value_witness_type(r);
                        // Pre-inference casts carry pure (or explicitly witness-
                        // wrapped) targets; re-derive the full target type from
                        // the inferred witness shape of the result.
                        let new_target = apply_witness_type(target.strip_all_witness(), r_wt);
                        OpCode::Cast {
                            result: r,
                            value,
                            target: new_target,
...
/// Append conversion casts emitted at a typed-slot boundary. Casts are pure
/// and total, so they are safe to execute unconditionally even in guarded
/// (tainted) regions; they are pushed unguarded.
fn flush_conversion_instrs(
    instrs: &mut Vec<OpCode>,
    _taint: Option<ValueId>,
    cast_instrs: Vec<OpCode>,
) {
    instrs.extend(cast_instrs);
}
...
fn emit_merge_select(
    ...
    match &result_type.expr {
        ...
        TypeExpr::WitnessOf(_) => {
            // Cast operands to the witness-typed merge type if they aren't already
            let lhs = if !lhs_type.is_witness_of() {
                builder.cast_to(result_type.clone(), lhs)
            } else {
                lhs
            };
            let rhs = if !rhs_type.is_witness_of() {
                builder.cast_to(result_type.clone(), rhs)
            } else {
                rhs
            };
            let result = result.unwrap_or_else(|| builder.fresh_value());
