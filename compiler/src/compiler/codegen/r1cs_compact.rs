...
//! - A row with an identically constant *multiplicand* has a linear product, so the whole row is a
//!   linear relation: `A ≡ 0` gives `C·w = 0`, and `A ≡ k·w_0` gives `(k·B − C)·w = 0` for an
//!   arbitrary `B` (symmetrically when `B` is the constant side). Solving such a relation for its
...
impl Substitution {
    ...
    /// Canonicalize one LC under the current substitution. The R1CS holds raw coefficients, so this
    /// is the boundary where they become middle-end [`Field`] values.
    fn apply(&mut self, lc: &LC, interner: &mut CoeffInterner) -> CanonLc {
        self.fold_terms(
            lc.iter()
                .map(|&(col, coeff)| (col as u32, Field::from(coeff))),
            interner,
        )
    }
