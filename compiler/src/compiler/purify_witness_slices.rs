        ...
        for located in old_instrs {
            ...
            match op {
                    ...
                    let (physical, log_len, start) = {
                        ...
                        match (replacement_tuple_map.get(&slice).copied(), dir) {
                            (Some(t), SliceOpDir::Back) => {
                                ...
                                for value in &values {
                                    let grown =
                                        b.slice_push(physical, vec![*value], SliceOpDir::Back);
                            ...
                            (Some(t), SliceOpDir::Front) => {
                                ...
                                let st = b.tuple_proj(t, 2);
                                let mut physical =
                                    b.slice_push(p, values.clone(), SliceOpDir::Front);
                            ...
                            (None, SliceOpDir::Front) => {
                                let base_len = b.slice_len(slice);
                                let physical =
                                    b.slice_push(slice, values.clone(), SliceOpDir::Front);
                                let zero = b.u_const(32, 0);
                                (physical, b.add(base_len, bump), zero)
                            }
                        }
                ...
                OpCode::SlicePop {
                    ...
                    result_elem,
                    slice,
                } if is_wl_slice(result_slice, affected) => {
                    let phys_ty = type_info.get_value_type(result_slice).clone();
