...
impl CSE {
    ...
    fn gather_expressions(
        ...
        // Each arm below still matches an operation's two signed forms together, but the ones where
        // the sign changes the _value_ now carry it into the interner node, so they no longer share
        // a key. The opcode is the authority on signedness.
        //
        // Every arm computes its discriminator with the same expression,
        // `kind.sign_changes_value().then(|| kind.is_signed())`, rather than deciding per operation.
        // `CmpKind::is_ordering` is the comparison counterpart: the `ULt | SLt` arm carries its
        // sign, `Eq` has none to carry.
