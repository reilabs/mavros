...
//!
//! Constant-valued block parameters are aliased but left in place; DCE prunes the now-unused
...
//! and result types never change. Integer folds are deliberately conservative: an op is folded only
//! when the mathematical result fits the operand bit width, because the backends do not agree on a
//! wrap width for overflowing pure integer ops. Division by a constant zero is likewise left alone.
...
impl SCCP {
    ...
    pub fn do_run(&self, ssa: &mut HLSSA) {
        ...
        for fid in fids {
            let function = ssa.take_function(fid);
...
fn join(a: Lattice, b: Lattice) -> Lattice {
    match (a, b) {
        (Lattice::Top, x) | (x, Lattice::Top) => x,
        (Lattice::Bottom, _) | (_, Lattice::Bottom) => Lattice::Bottom,
        (Lattice::Const(c1), Lattice::Const(c2)) => {
            if c1 == c2 {
                Lattice::Const(c1)
            } else {
                Lattice::Bottom
...
impl LatticeResult {
    fn lookup(&self, v: ValueId) -> Lattice {
        if let Some(c) = self.consts.get(&v) {
            return Lattice::Const((**c).clone());
        }
        self.lattice.get(&v).cloned().unwrap_or(Lattice::Top)
    }
}
...
impl<'f> FunctionLattice<'f> {
    fn new(function: &'f HLFunction, consts: HLSSAConstantsSnapshot) -> Self {
    ...
    fn into_result(self) -> LatticeResult {
        LatticeResult {
            consts: self.consts,
    ...
    fn run(&mut self) {
        ...
        for (p, _) in self.function.get_entry().get_parameters() {
            self.lattice.insert(*p, Lattice::Bottom);
        }
    ...
    fn force_stuck_conditions(&mut self) -> bool {
        ...
        for bid in &self.reachable {
            ...
            {
                if self.lattice_of(*cond) == Lattice::Top {
                    stuck.push(*cond);
                }
            }
        }
        for cond in &stuck {
            self.lattice.insert(*cond, Lattice::Bottom);
            self.value_worklist.push(*cond);
        }
        !stuck.is_empty()
    }

    fn lattice_of(&self, v: ValueId) -> Lattice {
        if let Some(c) = self.consts.get(&v) {
            return Lattice::Const((**c).clone());
        }
        self.lattice.get(&v).cloned().unwrap_or(Lattice::Top)
    ...
    fn set_lattice(&mut self, v: ValueId, new: Lattice) {
    ...
    fn process_value(&mut self, v: ValueId) {
        ...
        for (bid, site) in sites {
            ...
            match site {
                ...
                None => self.visit_terminator(bid),
            }
        }
    }
    ...
    fn visit_terminator(&mut self, bid: BlockId) {
        ...
        match term {
            ...
            Terminator::JmpIf(cond, t, f) => match self.lattice_of(cond) {
                Lattice::Top => {}
                Lattice::Const(c) => match const_bool(&c) {
    ...
    fn transfer(&self, instr: &OpCode) -> Vec<(ValueId, Lattice)> {
        match instr {
            ...
            // always the output of one of the pure scalar ops above.
            _ => instr.get_results().map(|r| (*r, Lattice::Bottom)).collect(),
        }
    }

    fn eval1(&self, v: ValueId, f: impl FnOnce(&Constant) -> Option<Constant>) -> Lattice {
        match self.lattice_of(v) {
            Lattice::Top => Lattice::Top,
            Lattice::Const(c) => f(&c).map(Lattice::Const).unwrap_or(Lattice::Bottom),
    ...
    fn eval2(
        ...
        match (self.lattice_of(l), self.lattice_of(r)) {
            ...
            (Lattice::Const(a), Lattice::Const(b)) => {
                f(&a, &b).map(Lattice::Const).unwrap_or(Lattice::Bottom)
            }
            _ => Lattice::Bottom,
        }
    }

    fn eval_select(&self, cond: ValueId, if_t: ValueId, if_f: ValueId) -> Lattice {
        match self.lattice_of(cond) {
            ...
            Lattice::Bottom => join(self.lattice_of(if_t), self.lattice_of(if_f)),
        }
    }
}
...
/// Decode an `s`-bit two's-complement raw value.
fn decode_i(s: usize, raw: u128) -> i128 {
    debug_assert!(s >= 1 && s <= MAX_SUPPORTED_SIGNED_BITS);
    if (raw >> (s - 1)) & 1 == 1 {
        (raw as i128) - (1i128 << s)
    } else {
        raw as i128
...
fn fits_i(s: usize, v: i128) -> bool {
    v >= -(1i128 << (s - 1)) && v < (1i128 << (s - 1))
}

fn encode_i(s: usize, v: i128) -> u128 {
    (v as u128) & bit_mask(s)
}

/// Fold a binary arithmetic op. Integer results must fit the operand width: the backends do not
/// agree on a wrap width for overflowing pure integer ops, so an overflowing fold is refused
        ...
        (Constant::I(s1, x), Constant::I(s2, y)) => {
            ...
            match kind {
                ...
                Add | Sub | Mul | Div | Mod => {
                    ...
                    Some(Constant::I(s, encode_i(s, v)))
                }
                Shl | Shr => None,
            }
        }
...
fn eval_cmp(kind: CmpKind, a: &Constant, b: &Constant) -> Option<Constant> {
    ...
    match (kind, a, b) {
        ...
        {
            res(decode_i(*s1, *x) < decode_i(*s1, *y))
        }
        _ => None,
    }
}
        ...
        for instr in instructions {
            ...
            // pure scalar folds (see `transfer`): its uses are aliased, so drop it.
            {
                ...
                if let (Some(r), None) = (results.next(), results.next()) {
                    if matches!(res.lattice.get(r), Some(Lattice::Const(_))) {
                        continue;
                    }
                }
            }
            ...
            {
                if let Lattice::Const(c) = res.lookup(*cond) {
