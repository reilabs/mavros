...
fn lower_type(ty: &HLType) -> LLType {
    match &ty.expr {
        ...
        HLTypeExpr::Array(..) | HLTypeExpr::Slice(_) => LLType::Ptr,
...
fn elem_struct(ty: &HLType) -> LLStruct {
    match &ty.expr {
        ...
        HLTypeExpr::Array(..) | HLTypeExpr::Slice(_) => LLStruct::new(vec![LLFieldType::Ptr]),
        HLTypeExpr::Tuple(_) => ice_non_elided_tuple(),
...
fn tuple_field_type(ty: &HLType) -> LLFieldType {
    match &ty.expr {
        ...
        HLTypeExpr::Array(..) | HLTypeExpr::Slice(_) => LLFieldType::Ptr,
        HLTypeExpr::Tuple(_) => ice_non_elided_tuple(),
...
fn sequence_data_field(ty: &HLType) -> usize {
    match &ty.expr {
        HLTypeExpr::Array(..) | HLTypeExpr::Slice(_) => SEQ_DATA_FIELD,
        _ => panic!("Expected array or slice type, got: {}", ty),
    }
}
...
fn lower_rc_bump(
    ...
    let rc_struct = match &val_type.expr {
        HLTypeExpr::Array(..) | HLTypeExpr::Slice(_) => sequence_rc_struct(val_type),
        HLTypeExpr::Tuple(_) => ice_non_elided_tuple(),
        HLTypeExpr::Ref(inner) => rc_ref_cell_struct(inner),
...
fn generate_all_drop_functions(llssa: &mut LLSSA, drop_fns: &[DropFnEntry]) {
    for entry in drop_fns {
        let func = match &entry.ty.expr {
            HLTypeExpr::Array(..) | HLTypeExpr::Slice(_) => {
