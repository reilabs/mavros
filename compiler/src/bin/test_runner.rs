...
fn run_single(root: PathBuf, expect_failure: bool, analyze: bool) {
    ...
    let r1cs = match driver.generate_r1cs() {
        Ok(r) => {
            ...
            // post-seal compaction pass could remove. It is _not_ part of STATUS.md — the parent
        ...
        // A program whose assertion can never hold is rejected as unsatisfiable while generating
...
fn run_parent(output_path: &Path, jobs: usize, ignored_tests: &[&str], analyze: bool) {
    ...
    // 2. Local programs expected to be rejected — at compile time (frontend or Mavros-specific)
    // or at witness generation (a bounds check that only a real input can violate). Keeping these
    // separate from `noir_tests` lets the same child runner exercise them without treating a
    // clean rejection as a regression.
...
/// what must never happen silently is a _stage_ column going missing entirely — e.g. a renamed
