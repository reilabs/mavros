...
`std::mem::array_refcount` and `std::mem::vector_refcount` return zero in constrained code and read
Mavros' current allocation reference count in unconstrained code. These are debugging operations:
exact counts depend on the backend and optimizations. For arrays or vectors of tuples and structs,
Mavros reports the first backing allocation's count after splitting the fields into separate arrays.
Values erased entirely by that split, such as arrays of empty structs, return zero.
