...
//
// `OPCODE_PROFILE` / `OPCODE_NAMES` are generated by the `#[interpreter]` macro
// alongside the dispatch table. The generated handlers read the cycle counter
// at entry/exit and accumulate `(count, cycles)` per opcode. The helpers below
// reset and pretty-print that data; `read_cycles` is the timer source.
// ---------------------------------------------------------------------------
...
/// Zero all per-opcode counters. Call before a dispatch run to isolate it.
#[cfg(feature = "vm-profile")]
pub fn reset_opcode_profile() {
    use core::sync::atomic::Ordering::Relaxed;
    for entry in OPCODE_PROFILE.iter() {
        entry.0.store(0, Relaxed);
        entry.1.store(0, Relaxed);
    }
}

/// Print a per-opcode profiling report to stderr, sorted by accumulated cycles.
#[cfg(feature = "vm-profile")]
pub fn report_opcode_profile(label: &str) {
    use core::sync::atomic::Ordering::Relaxed;

    let mut rows: Vec<(&str, u64, u64)> = OPCODE_NAMES
        .iter()
        .zip(OPCODE_PROFILE.iter())
        .map(|(name, entry)| (*name, entry.0.load(Relaxed), entry.1.load(Relaxed)))
