...
**Phase 3 Partially Complete:** Most of Phase 3 is already in the tree: a `FieldConfig` is homed on
the `SSA` (`ssa/mod.rs`) and reached through `ssa.field()` / `b.field()` / the `HLEmitter::field()`
trait method, three of the four symbolic evaluators (click_cooper's lattice, the specializer, the
instrumenter) plus the value-range interval algebra and the PRE totality oracle route their field
values through it, the six `two_pow` copies are down to one, and no middle-end site names a static
`Field::` associated item or a literal prime. The box stays unticked for:

- The **fourth evaluator**, `codegen/hlssa_to_r1cs.rs`, which deliberately stays on raw
  `ark_bn254::Fr` — codegen is the raw side of the boundary and learns its field in P4 with the
  per-field opcode set;
- The remaining `L3-width254` literals in `hlssa/type_system.rs` and
  `lowering/expression_converter.rs`, which have no `SSA` in scope at the point of use;
- The `--field` selector that will seed `SSA::field` from the `Driver` (P5).
...
- [ ] `compiler/src/compiler/analysis/symbolic_executor.rs:100` —
      ...
      the specializer, an `OpInstrumenter::field()` trait method for the instrumenter, and a `field`
      member on click_cooper's `FunctionSolver`. Revisit only if a consumer appears that has the
      `ctx` but none of those.
...
- [x] `compiler/src/compiler/passes/instruction_lowering/bit_range.rs` — `assert!(bits <= 254, ...)`
      → `assert!(bits <= b.field().field_bit_size() as usize, ...)` (both sites). The two above have
      no `SSA`/emitter in scope and still need one.
...
- [ ] `compiler/src/compiler/codegen/hlssa_to_r1cs.rs` — `Value` impl (enum + impl); LC coefficient
      arithmetic `add/neg/mul/div`. The fourth evaluator, deliberately still raw — see the Phase 3
      note at the top.
...
that gives each field its own implementation, not as outstanding P3 work.
...
associated item. The residual on the first two is that `modulus_limbs()` still returns `[u64; 4]`,
so the limb-count assumption is moved rather than erased.
